 ...
 */
 ...
 namespace Mosframe {
    ...
    public abstract class DynamicScrollView : UIBehaviour {
	    ...
	    private int             totalItemCount   = ProfileConfig.k_maxLevel/4;
	    public RectTransform   itemPrototype    = null;

        public static UnityEvent refreshEvent;
        ...
        protected virtual IEnumerator onSeedData() {
            ...
            var itemCount = (int)(this.viewportSize / this.itemSize) + 10;

		    for( var i = 0; i < itemCount; ++i ) {

			    var itemRect = Instantiate( this.itemPrototype , itemPrototype.transform.parent, false);
            ...
            this.scrollByItemIndex(PlayerManager.Instance.m_currentLevel/4);
        }
	    ...
	    private void Update () {
		    ...
		    while( this.contentAnchoredPosition - this.prevAnchoredPosition  < -this.itemSize * 8 ) {
	    ...
	    private void updateItem ( int index, GameObject itemObj ) {
		    ...
		    if( index < 0 || index >= this.totalItemCount ) {
			    ...
			    var item = itemObj.GetComponent<Group4Level>();
                if( item != null ) item.SetupLevel( index );
		    }
	    }
